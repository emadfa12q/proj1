...
public class UsageTrackerService extends Service {
    ...
    private void tick() {
        ...
        if (pkg == null || pkg.trim().isEmpty() || pkg.equals(getPackageName())) {
            updateNotification("در حال ثبت فعالیت…");
            return;
        }
        ...
        updateNotification("در حال ثبت: " + currentAppName);
    ...
    private String latestForegroundPackage() {
        ...
        long now = System.currentTimeMillis();
        long from = Math.max(0, now - 15L * 60L * 1000L);
    ...
    private void closeCurrent(long end) {
        if (!currentPackage.isEmpty() && currentStart > 0 && end > currentStart) {
            db.insertSegment(currentStart, end, currentAppName, currentPackage);
        }
        currentPackage = "";
        currentAppName = "";
        currentStart = 0L;
